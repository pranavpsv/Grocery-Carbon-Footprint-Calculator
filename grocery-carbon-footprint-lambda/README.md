...
```bash
grocery-carbon-footprint-lambda$ sam build
```
...
```bash
grocery-carbon-footprint-lambda$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
grocery-carbon-footprint-lambda$ sam local start-api
grocery-carbon-footprint-lambda$ curl http://localhost:3000/inference
```
...
```bash
grocery-carbon-footprint-lambda$ sam logs -n InferenceFunction --stack-name grocery-carbon-footprint-lambda --tail
```
...
```bash
aws cloudformation delete-stack --stack-name grocery-carbon-footprint-lambda
```
